...
function menuForCoords(lat,lon){
  ...
  // Constantine + east → Menu 3 (interior)
  // Annaba = Menu 2
  if(distKm(lat,lon,36.9,7.77)<40)return'2';
  // Menu 5 = Khemis Miliana / Aïn Defla
  if(distKm(lat,lon,36.26,2.22)<35)return'5';
  // Annaba = Menu 2
  if(distKm(lat,lon,36.9,7.77)<40)return'2';
  // Menu 5 = Khemis/Aïn Defla
  if(distKm(lat,lon,36.26,2.22)<35)return'5';
  // Menu 4 = Blida
  if(distKm(lat,lon,36.47,2.83)<25)return'4';
...
async function initPriceTabs(){
  ...
  const LABELS={'1':'Alger & région','2':'Oran · Mostaganem · Annaba','3':'Constantine · Djelfa · Chlef','4':'Blida','5':'Khemis Miliana · Aïn Defla'};

  function activate(m,save){
    if(!['1','2','3','4','5'].includes(String(m)))m='1';
